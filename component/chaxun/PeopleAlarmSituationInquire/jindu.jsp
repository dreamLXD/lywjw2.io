...
<!--  || '-1'  eq record.bhpd  备用判断    -->
         <li class="layui-timeline-item" <c:if test="${ record.sptype!=2  ||'-1'  ne record.bhpd}">style="display: none" </c:if>>
        <i class="layui-icon layui-timeline-axis">&#xe63f;</i>
        <div class="layui-timeline-content layui-text">
...
<script>
    $(function () {
        $('.pj').on('click', function () {
            window.location.href = "<%=basePath%>wx/Query/Survey?openid=${openid}&id=${id}";
        })
    })
</script>
