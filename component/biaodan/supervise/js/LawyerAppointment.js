...
//	$('.button_type input').on('click',function(){
//		if($(this).val()=='看守所'){
//			onebutton()
//		}else if($(this).val()=='拘留所'){
//			
//		}
//	})
...
$(function () {
	...
	$('footer button').on('click', function (e) {
		...
		arr.forEach((item) => {
			...
			if (item.name.match('AccompanyingName')) {
				newstr += item.value + ','
			}
			if (item.name.match('AccompanyingCard')) {
				newstr += item.value + '||'
			}
		...
		filestr('arr_file1', '.one', 'onefile','3098a0cf46154c1285872624aa9ad129')
		filestr('arr_file2', '.two', 'twofile','bb7c5b0149c942b38c0ca559f3abf670')
		filestr('arr_file3', '.three', 'threefile','287ad4e4412f40e19cc8e76a073f7ec0')
			...
			if(key){
				$.confirm({
					  ...
					  onOK: function () {
							jiazai()
							$.ajax({
								url: base_path +'/supervise/LawyerAppointment',
								type: 'post',
								data: obj,
								dataType: 'json',
								//async: false,
		...
		$('.ptrxx .tianjia').on('click', function () {
    		...
    		<div class="weui-cells weui-cells_form">
					<div class="weui-cell">
						<div class="weui-cell__hd"><label class="weui-label">陪同人员姓名:</label><span class="del"></span></div>
						<div class="weui-cell__bd">
							<input class="weui-input" required name="AccompanyingName${num}" placeholder="请输入姓名">
						</div>
					</div>
					<div class="weui-cell">
						<div class="weui-cell__hd"><label class="weui-label">陪同人员身份证号:</label></div>
						<div class="weui-cell__bd">
							<input class="weui-input" required name="AccompanyingCard${num}" placeholder="请输入身份证号码">
						</div>
					</div>
			</div>
			`
