...
$(function () {
	...
	$('footer button').on('click', function (e) {
		...
		arr.forEach((item) => {
			...
			if (item.name == 'RelationsDetainees') {
                obj[item.name] = $('#RelationsDetainees').data('values')
			}
		...
		function filestr(arr_file, ul, onefile,id) {
			...
			let str_arr = '195cf1772fdf45e98973fdef322e32c7';
		...
		obj.blyw='195cf1772fdf45e98973fdef322e32c7'
			...
			if(key){
				$.confirm({
					  ...
					  onOK: function () {
							jiazai();
							$.ajax({
								url: base_path +'/supervise/FamilyAppointment',
								type: 'post',
								data: obj,
								dataType: 'json',
								//async: false,
